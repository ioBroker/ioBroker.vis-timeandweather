 ...
 *     npm run screenshots -- flip-clock weather     (only these)
 *
...
const SCALE = 2;
// Taller than the page, so every section is laid out like on a real screen
...
const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'tw-shots-'));
const chrome = spawn(
    findChrome(),
    [
        '--headless',
...
try {
    ...
    if (pageHeight > VIEWPORT.height) {
        console.warn(`The page (${pageHeight}px) is taller than the window`);
    }
