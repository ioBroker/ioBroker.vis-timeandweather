/**
 * Skins of the CoolClock, taken over unchanged from `widgets/timeandweather/js/moreskins.js` of the vis-1 set.
 *
 * CoolClock by Simon Baird (MIT), https://github.com/simonbaird/CoolClock - skins by their authors as noted there.
 * All positions are in "render units": the clock face has a radius of 100, whatever its size on the screen.
 */

export interface CoolClockPart {
    lineWidth: number;
    /** Radius of a circle; if set, the part is drawn as a circle at `startAt` instead of as a line */
    radius?: number;
    startAt?: number;
    endAt?: number;
    color?: string;
...
export interface CoolClockSkin {
    outerBorder?: CoolClockPart;
    smallIndicator?: CoolClockPart;
    largeIndicator?: CoolClockPart;
    hourHand?: CoolClockPart;
    minuteHand?: CoolClockPart;
    secondHand?: CoolClockPart;
    hourDecoration?: CoolClockPart;
    minDecoration?: CoolClockPart;
    secondDecoration?: CoolClockPart;
    /** Canvas font of the digital time */
    font?: string;
    /** Colour of the digital time */
    fontColor?: string;
}

/** Used when the configured skin does not exist, like `CoolClock.config.defaultSkin` */
export const DEFAULT_SKIN = 'chunkySwiss';

export const COOL_CLOCK_SKINS: Record<string, CoolClockSkin> = {
    swissRail: {
    ...
    classicWhite: {
        ...
        secondHand: { lineWidth: 1, startAt: -20, endAt: 85, color: '#FFCFCF', alpha: 0.95 },
        secondDecoration: { lineWidth: 3, startAt: 0, radius: 2, fillColor: 'white', color: 'white', alpha: 1 },
        // vis-1 had `{ 'font-family': 'arial', color: '#FFFFFF', 'font-size': 14 }` here, which the canvas ignored
        font: '14px arial',
        fontColor: '#FFFFFF',
    },
};
